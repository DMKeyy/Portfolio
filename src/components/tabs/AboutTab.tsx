...
const experience = {
  ...
  useEffect(() => {
    // Calculate total lines including profile card height
    const profileCardLines = 20; // Approximate number of lines for the profile card
    const codeLines = fullText.split('\n').length + educationExpText.split('\n').length;
    const totalLines = profileCardLines + codeLines;
    setLineNumbers(Array.from({ length: totalLines }, (_, i) => i + 1));

    // Typing animation for initial code
    let index = 0;
    const timer = setInterval(() => {
      if (index <= fullText.length) {
        ...
        clearInterval(timer);
        // Start typing education and experience after initial code finishes
        let eduExpIndex = 0;
      ...
      <div className="flex flex-1 overflow-y-auto">
        ...
        <div className="flex-1 p-4 font-mono text-sm">
          ...
          <motion.div
            ...
            className="mb-8 bg-[#1e1e1e] rounded-lg border border-[#2d2d30] p-6 w-full max-w-[600px] mx-auto cursor-pointer transition-all duration-300 hover:border-[#0078d4] relative overflow-hidden group"
          >
            {/* Gradient Overlay */}
            <motion.div 
